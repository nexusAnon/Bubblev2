...
_Run: 2026-08-10T07:59:03 — 46 passed, 0 failed, 0 skipped._

---

## ✓ a fresh BUBBLE_HOME yields a usable vault DB on schema v2

`00_sanity/test_vault_initializes.py` — 42 ms

```
vault_db: /tmp/bubble-test-rj9whxc8/vault.db
...
`10_breakers/test_agent_polyglot.py` — 151 ms

```
AgentVault registered and executed isolated C tool successfully
AgentVault registered and executed isolated Bash tool successfully
AgentVault universal v4 metadata registration verified inside database
```
...
`10_breakers/test_agent_vault_embedding.py` — 158 ms
...
AgentVault(home=av-second-home-ho_c2i67) created separate vault root with its own SQLite index
```
...
`10_breakers/test_bundle_round_trip.py` — 213 ms

```
bundled: 2 packages, 15 files, 2168 bytes
...
`10_breakers/test_empty_script_fetches_nothing.py` — 57 ms
...
```
flatmod_old.__file__: /tmp/bubble-test-fl3ke_u1/vault/flatmod/1.0.0/py3-none-any/flatmod.py
flatmod_new.__file__: /tmp/bubble-test-fl3ke_u1/vault/flatmod/2.0.0/py3-none-any/flatmod.py
...
`10_breakers/test_https_index_required.py` — 63 ms
...
module id stable:  0x7f0bb6c0e430 → 0x7f0bb6c0e430
class id stable:   0x55df929b6b60 → 0x55df929b6b60
STATE dicts distinct: old@0x7f0bb6c16840 vs new@0x7f0bb6c17140
...
`10_breakers/test_metadata_per_alias.py` — 126 ms
...
`10_breakers/test_modules_and_deps_indexed.py` — 73 ms
...
`10_breakers/test_recursive_self_host.py` — 905 ms

```
build script: tools/build_pyz.py
bubble run tools/build_pyz.py: rc=0
produced artifact: 136895 bytes
sidecar sha256 matches bytes: 1ed1f8f1828fe71a…
produced pyz --help responds and lists bubble subcommands
produced pyz `vault list` returned: 'vault is empty'
deterministic: two builds same source → identical sha256 1ed1f8f1828fe71a…
```
...
`10_breakers/test_sdist_refused_by_default.py` — 614 ms
...
`10_breakers/test_shell_create_follows_requires_dist.py` — 124 ms

```
shell at /tmp/bubble-test-z6f51l1a/shells/closuretest
...
`10_breakers/test_shell_create_from_manifest.py` — 115 ms
...
```
shell at /tmp/bubble-test-ckplast4/shells/distinfotest
...
`10_breakers/test_shell_create_merges_namespace_packages.py` — 189 ms

```
shell at /tmp/bubble-test-klv2vfbo/shells/nstest
...
`10_breakers/test_shell_create_refuses_orphan_targets.py` — 102 ms
...
`10_breakers/test_top_level_carries_content_hash.py` — 71 ms
...
`10_breakers/test_top_level_contention_logged.py` — 74 ms
...
`10_breakers/test_top_level_index_bridges_import_to_dist.py` — 79 ms

```
distribution name: Carbohydrate-9000
top-level import:  sugar
top_level row:     ('Carbohydrate-9000', '3.0.0', 'sugar')
resolved module:   /tmp/bubble-test-qiw34k_k/vault/Carbohydrate-9000/3.0.0/py3-none-any/sugar/__init__.py
...
`10_breakers/test_two_versions_one_process.py` — 89 ms

```
widget_old.Widget: id=0x56253b9eb2b0
widget_new.Widget: id=0x56253b9ed100
...
`10_breakers/test_universal_ingest.py` — 198 ms
...
`10_breakers/test_universal_shell.py` — 62 ms
...
`10_breakers/test_universal_v4.py` — 47 ms
...
```
  bubble-test-_u45yn9x mode=0o700
  vault              mode=0o700
  .staging           mode=0o700
  shells             mode=0o700
  wheels             mode=0o700
...
`30_loop/test_agent_vault_isolation.py` — 197 ms
...
`30_loop/test_dlmopen_routing_through_proxy.py` — 163 ms
...
staged islet==3.1.4 at /tmp/bubble-test-ol5x8u8x/vault/islet/3.1.4/py3-none-any
...
```
probed_at: 2026-08-10T07:59:01.246603
...
`30_loop/test_subprocess_routing_through_proxy.py` — 192 ms
...
`30_loop/test_subprocess_substrate_handler.py` — 152 ms
...
staged islet_sub==3.1.4 at /tmp/bubble-test-jd3q0yhj/vault/islet_sub/3.1.4/py3-none-any
...
`30_loop/test_substrate_routing_learns.py` — 107 ms
...
`70_project/test_project_freeze.py` — 110 ms
...
`70_project/test_project_ingest_basic.py` — 100 ms
...
`70_project/test_project_parent_chain.py` — 120 ms

```
spinoff delta: ('delta', '3.0.0', 'py3-none-any', '/tmp/bubble-test-8qq8kk_h/vault/delta/3.0.0/py3-none-any')
spinoff echo_pkg (inherited): ('echo-pkg', '4.0.0', 'py3-none-any', '/tmp/bubble-test-8qq8kk_h/vault/echo-pkg/4.0.0/py3-none-any')
```

## ✓ ingest persists package scope to shell metadata

`70_project/test_project_scope_persisted.py` — 112 ms
...
`70_project/test_project_soft_isolation.py` — 124 ms
