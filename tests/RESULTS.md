...
_Run: 2026-08-02T16:43:44 — 42 passed, 0 failed, 0 skipped._
...
```
vault_db: /tmp/bubble-test-ydv0ozp0/vault.db
...
`10_breakers/test_agent_vault_embedding.py` — 130 ms
...
AgentVault(home=av-second-home-a49kwkk2) created separate vault root with its own SQLite index
```
...
`10_breakers/test_bundle_round_trip.py` — 171 ms

```
bundled: 2 packages, 15 files, 2029 bytes
...
`10_breakers/test_canonical_name_validated.py` — 604 ms
...
`10_breakers/test_diff_primitive.py` — 114 ms
...
`10_breakers/test_drift_refuses_and_records.py` — 76 ms
...
`10_breakers/test_empty_script_fetches_nothing.py` — 54 ms
...
`10_breakers/test_fetcher_refuses_off_host_url.py` — 58 ms
...
`10_breakers/test_flat_module_alias.py` — 86 ms

```
flatmod_old.__file__: /tmp/bubble-test-lrmmknd4/vault/flatmod/1.0.0/py3-none-any/flatmod.py
flatmod_new.__file__: /tmp/bubble-test-lrmmknd4/vault/flatmod/2.0.0/py3-none-any/flatmod.py
...
`10_breakers/test_https_index_required.py` — 61 ms
...
`10_breakers/test_importer_refuses_symlinks.py` — 60 ms
...
`10_breakers/test_late_alias_does_not_corrupt_earlier.py` — 78 ms
...
module id stable:  0x7f5eb79a63e0 → 0x7f5eb79a63e0
class id stable:   0x55bc008973a0 → 0x55bc008973a0
STATE dicts distinct: old@0x7f5eb79ae700 vs new@0x7f5eb79af000
...
`10_breakers/test_metadata_per_alias.py` — 113 ms
...
`10_breakers/test_recursive_self_host.py` — 840 ms

```
build script: tools/build_pyz.py
bubble run tools/build_pyz.py: rc=0
produced artifact: 126757 bytes
sidecar sha256 matches bytes: 30e82ef18c581a3c…
produced pyz --help responds and lists bubble subcommands
produced pyz `vault list` returned: 'vault is empty'
deterministic: two builds same source → identical sha256 30e82ef18c581a3c…
```
...
`10_breakers/test_sdist_refused_by_default.py` — 609 ms
...
`10_breakers/test_shell_create_follows_requires_dist.py` — 108 ms

```
shell at /tmp/bubble-test-2wz7gx91/shells/closuretest
...
`10_breakers/test_shell_create_links_dist_info.py` — 196 ms

```
shell at /tmp/bubble-test-4fpvld8x/shells/distinfotest
...
`10_breakers/test_shell_create_merges_namespace_packages.py` — 179 ms

```
shell at /tmp/bubble-test-kx7dxcm2/shells/nstest
...
`10_breakers/test_shell_create_refuses_orphan_targets.py` — 87 ms
...
`10_breakers/test_top_level_carries_content_hash.py` — 60 ms
...
`10_breakers/test_top_level_contention_logged.py` — 61 ms
...
`10_breakers/test_top_level_index_bridges_import_to_dist.py` — 83 ms

```
distribution name: Carbohydrate-9000
top-level import:  sugar
top_level row:     ('Carbohydrate-9000', '3.0.0', 'sugar')
resolved module:   /tmp/bubble-test-usc9crrv/vault/Carbohydrate-9000/3.0.0/py3-none-any/sugar/__init__.py
...
`10_breakers/test_top_level_verify_mode.py` — 57 ms
...
`10_breakers/test_two_versions_one_process.py` — 70 ms

```
widget_old.Widget: id=0x563786fbde30
widget_new.Widget: id=0x563786fbe520
...
```
  bubble-test-p9g7fx7c mode=0o700
  vault              mode=0o700
  .staging           mode=0o700
  shells             mode=0o700
  wheels             mode=0o700
...
`30_loop/test_agent_vault_isolation.py` — 177 ms
...
`30_loop/test_dlmopen_routing_through_proxy.py` — 145 ms
...
`30_loop/test_dlmopen_substrate_handler.py` — 94 ms
...
staged islet==3.1.4 at /tmp/bubble-test-5o8b4ns4/vault/islet/3.1.4/py3-none-any
...
```
probed_at: 2026-08-02T16:43:42.378612
...
`30_loop/test_subprocess_routing_through_proxy.py` — 162 ms
...
`30_loop/test_subprocess_substrate_handler.py` — 150 ms
...
staged islet_sub==3.1.4 at /tmp/bubble-test-69ws9sc6/vault/islet_sub/3.1.4/py3-none-any
...
`70_project/test_project_freeze.py` — 102 ms
...
`70_project/test_project_ingest_basic.py` — 84 ms
...
```
spinoff delta: ('delta', '3.0.0', 'py3-none-any', '/tmp/bubble-test-nre10bvs/vault/delta/3.0.0/py3-none-any')
spinoff echo_pkg (inherited): ('echo-pkg', '4.0.0', 'py3-none-any', '/tmp/bubble-test-nre10bvs/vault/echo-pkg/4.0.0/py3-none-any')
```
...
`70_project/test_project_soft_isolation.py` — 113 ms
