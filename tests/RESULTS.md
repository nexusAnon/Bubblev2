...
_Run: 2026-08-10T07:08:38 — 45 passed, 0 failed, 0 skipped._

---

## ✓ a fresh BUBBLE_HOME yields a usable vault DB on schema v2

`00_sanity/test_vault_initializes.py` — 35 ms

```
vault_db: /tmp/bubble-test-_ncxjnsu/vault.db
...
`10_breakers/test_agent_vault_embedding.py` — 132 ms
...
AgentVault(home=av-second-home-irdpcyg4) created separate vault root with its own SQLite index
```
...
`10_breakers/test_bundle_round_trip.py` — 177 ms

```
bundled: 2 packages, 15 files, 2167 bytes
...
`10_breakers/test_canonical_name_validated.py` — 106 ms
...
`10_breakers/test_diff_primitive.py` — 110 ms
...
`10_breakers/test_drift_refuses_and_records.py` — 74 ms
...
`10_breakers/test_empty_script_fetches_nothing.py` — 61 ms
...
`10_breakers/test_fetcher_refuses_off_host_url.py` — 51 ms
...
`10_breakers/test_flat_module_alias.py` — 78 ms

```
flatmod_old.__file__: /tmp/bubble-test-bcrd50pe/vault/flatmod/1.0.0/py3-none-any/flatmod.py
flatmod_new.__file__: /tmp/bubble-test-bcrd50pe/vault/flatmod/2.0.0/py3-none-any/flatmod.py
...
`10_breakers/test_https_index_required.py` — 58 ms
...
`10_breakers/test_importer_refuses_symlinks.py` — 58 ms
...
`10_breakers/test_late_alias_does_not_corrupt_earlier.py` — 70 ms
...
module id stable:  0x7f84ddd2a480 → 0x7f84ddd2a480
class id stable:   0x55f036ceeb60 → 0x55f036ceeb60
STATE dicts distinct: old@0x7f84ddd326c0 vs new@0x7f84ddd32fc0
...
`10_breakers/test_metadata_per_alias.py` — 107 ms
...
`10_breakers/test_modules_and_deps_indexed.py` — 62 ms
...
`10_breakers/test_recursive_self_host.py` — 902 ms

```
build script: tools/build_pyz.py
bubble run tools/build_pyz.py: rc=0
produced artifact: 135139 bytes
sidecar sha256 matches bytes: 6ac3ccb49a6a38ae…
produced pyz --help responds and lists bubble subcommands
produced pyz `vault list` returned: 'vault is empty'
deterministic: two builds same source → identical sha256 6ac3ccb49a6a38ae…
```
...
`10_breakers/test_sdist_refused_by_default.py` — 608 ms
...
`10_breakers/test_shell_create_follows_requires_dist.py` — 106 ms

```
shell at /tmp/bubble-test-zc_3vlbs/shells/closuretest
...
`10_breakers/test_shell_create_from_manifest.py` — 105 ms
...
`10_breakers/test_shell_create_links_dist_info.py` — 188 ms

```
shell at /tmp/bubble-test-dnq28100/shells/distinfotest
...
`10_breakers/test_shell_create_merges_namespace_packages.py` — 166 ms

```
shell at /tmp/bubble-test-hjve7aa2/shells/nstest
...
`10_breakers/test_shell_create_refuses_orphan_targets.py` — 81 ms
...
`10_breakers/test_top_level_carries_content_hash.py` — 65 ms
...
`10_breakers/test_top_level_contention_logged.py` — 66 ms
...
`10_breakers/test_top_level_index_bridges_import_to_dist.py` — 72 ms

```
distribution name: Carbohydrate-9000
top-level import:  sugar
top_level row:     ('Carbohydrate-9000', '3.0.0', 'sugar')
resolved module:   /tmp/bubble-test-3cd9g5ta/vault/Carbohydrate-9000/3.0.0/py3-none-any/sugar/__init__.py
...
`10_breakers/test_top_level_verify_mode.py` — 58 ms
...
`10_breakers/test_two_versions_one_process.py` — 79 ms

```
widget_old.Widget: id=0x55711de5c2b0
widget_new.Widget: id=0x55711de5e100
...
`10_breakers/test_universal_ingest.py` — 186 ms

```
C compilation and in-place link-patching verified inside the shell
Bash script dynamic ingestion and relative symlinking verified
Direct bubble run C compilation-execution verified
```

## ✓ Phase 3 universal environment operators and interactive prompt entry hooks execute with perfect correctness

`10_breakers/test_universal_shell.py` — 57 ms
...
`10_breakers/test_universal_v4.py` — 51 ms
...
```
  bubble-test-jph7eg7m mode=0o700
  vault              mode=0o700
  .staging           mode=0o700
  shells             mode=0o700
  wheels             mode=0o700
...
`30_loop/test_agent_vault_isolation.py` — 178 ms
...
`30_loop/test_dlmopen_routing_through_proxy.py` — 140 ms
...
`30_loop/test_dlmopen_substrate_handler.py` — 97 ms
...
staged islet==3.1.4 at /tmp/bubble-test-ycdtsnyv/vault/islet/3.1.4/py3-none-any
...
```
probed_at: 2026-08-10T07:08:36.460530
...
`30_loop/test_subprocess_routing_through_proxy.py` — 156 ms
...
`30_loop/test_subprocess_substrate_handler.py` — 135 ms
...
staged islet_sub==3.1.4 at /tmp/bubble-test-s8m7mw3e/vault/islet_sub/3.1.4/py3-none-any
...
`30_loop/test_substrate_routing_learns.py` — 73 ms
...
`70_project/test_project_freeze.py` — 80 ms
...
`70_project/test_project_ingest_basic.py` — 93 ms
...
`70_project/test_project_parent_chain.py` — 106 ms

```
spinoff delta: ('delta', '3.0.0', 'py3-none-any', '/tmp/bubble-test-36aongfh/vault/delta/3.0.0/py3-none-any')
spinoff echo_pkg (inherited): ('echo-pkg', '4.0.0', 'py3-none-any', '/tmp/bubble-test-36aongfh/vault/echo-pkg/4.0.0/py3-none-any')
```

## ✓ ingest persists package scope to shell metadata

`70_project/test_project_scope_persisted.py` — 90 ms
...
`70_project/test_project_soft_isolation.py` — 98 ms
