...
_Run: 2026-07-31T01:13:11 — 41 passed, 1 failed, 0 skipped._

---

## ✓ a fresh BUBBLE_HOME yields a usable vault DB on schema v2

`00_sanity/test_vault_initializes.py` — 60 ms

```
vault_db: /tmp/bubble-test-9g4s9d6v/vault.db
...
`10_breakers/test_agent_vault_embedding.py` — 180 ms
...
AgentVault(home=av-second-home-zy_hxgxb) created separate vault root with its own SQLite index
```
...
`10_breakers/test_bundle_round_trip.py` — 133 ms
...
Traceback (most recent call last):
  File "/app/tests/_common.py", line 149, in run_test
    fn(result)
  File "/app/tests/10_breakers/test_bundle_round_trip.py", line 71, in body
    summary = bundle_mod.bundle("ship", bundle_path)
              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/app/bubble/bundle.py", line 121, in bundle
    bundle_manifest = _emit_bundle_manifest(
                      ^^^^^^^^^^^^^^^^^^^^^^
  File "/app/bubble/bundle.py", line 216, in _emit_bundle_manifest
    f'host = "{config.detect_host()}"',
               ^^^^^^^^^^^^^^^^^^^^
  File "/app/bubble/config.py", line 60, in detect_host
    if Path("/etc/debian_version").exists() and Path("/proc/1/root").exists():
                                                ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/home/jules/.pyenv/versions/3.12.13/lib/python3.12/pathlib.py", line 860, in exists
    self.stat(follow_symlinks=follow_symlinks)
  File "/home/jules/.pyenv/versions/3.12.13/lib/python3.12/pathlib.py", line 840, in stat
...
`10_breakers/test_canonical_name_validated.py` — 633 ms
...
`10_breakers/test_diff_primitive.py` — 138 ms
...
`10_breakers/test_drift_refuses_and_records.py` — 86 ms
...
`10_breakers/test_empty_script_fetches_nothing.py` — 51 ms
...
`10_breakers/test_fetcher_refuses_off_host_url.py` — 72 ms
...
`10_breakers/test_flat_module_alias.py` — 116 ms

```
flatmod_old.__file__: /tmp/bubble-test-zuwccm8b/vault/flatmod/1.0.0/py3-none-any/flatmod.py
flatmod_new.__file__: /tmp/bubble-test-zuwccm8b/vault/flatmod/2.0.0/py3-none-any/flatmod.py
...
`10_breakers/test_https_index_required.py` — 82 ms
...
`10_breakers/test_importer_refuses_symlinks.py` — 68 ms
...
`10_breakers/test_late_alias_does_not_corrupt_earlier.py` — 115 ms
...
module id stable:  0x7f6a5199a3e0 → 0x7f6a5199a3e0
class id stable:   0x561f479b5600 → 0x561f479b5600
STATE dicts distinct: old@0x7f6a519a2540 vs new@0x7f6a519a2e40
...
`10_breakers/test_metadata_per_alias.py` — 141 ms
...
`10_breakers/test_modules_and_deps_indexed.py` — 95 ms
...
`10_breakers/test_recursive_self_host.py` — 1007 ms

```
build script: tools/build_pyz.py
bubble run tools/build_pyz.py: rc=0
produced artifact: 126527 bytes
sidecar sha256 matches bytes: 945b48f31cb2d587…
produced pyz --help responds and lists bubble subcommands
produced pyz `vault list` returned: 'vault is empty'
deterministic: two builds same source → identical sha256 945b48f31cb2d587…
```
...
`10_breakers/test_run_default_no_network.py` — 12 ms
...
`10_breakers/test_sdist_refused_by_default.py` — 658 ms
...
`10_breakers/test_shell_create_follows_requires_dist.py` — 155 ms

```
shell at /tmp/bubble-test-0lw0orl6/shells/closuretest
...
`10_breakers/test_shell_create_from_manifest.py` — 137 ms
...
`10_breakers/test_shell_create_links_dist_info.py` — 207 ms

```
shell at /tmp/bubble-test-_yyzrrru/shells/distinfotest
...
`10_breakers/test_shell_create_merges_namespace_packages.py` — 216 ms

```
shell at /tmp/bubble-test-2gsqs97k/shells/nstest
...
`10_breakers/test_top_level_carries_content_hash.py` — 81 ms
...
`10_breakers/test_top_level_contention_logged.py` — 80 ms
...
`10_breakers/test_top_level_index_bridges_import_to_dist.py` — 92 ms

```
distribution name: Carbohydrate-9000
top-level import:  sugar
top_level row:     ('Carbohydrate-9000', '3.0.0', 'sugar')
resolved module:   /tmp/bubble-test-o0ehf2un/vault/Carbohydrate-9000/3.0.0/py3-none-any/sugar/__init__.py
...
`10_breakers/test_top_level_verify_mode.py` — 73 ms
...
`10_breakers/test_two_versions_one_process.py` — 91 ms

```
widget_old.Widget: id=0x55f805cb40a0
widget_new.Widget: id=0x55f805cd3c40
...
```
  bubble-test-ew6b3xo_ mode=0o700
  vault              mode=0o700
  .staging           mode=0o700
  shells             mode=0o700
  wheels             mode=0o700
...
`30_loop/test_agent_vault_isolation.py` — 184 ms
...
staged islet==3.1.4 at /tmp/bubble-test-d71nn9k7/vault/islet/3.1.4/py3-none-any
...
`30_loop/test_failure_recording_round_trip.py` — 26 ms
...
`30_loop/test_probe_writes_host_toml.py` — 24 ms

```
probed_at: 2026-07-31T01:13:09.292859
...
`30_loop/test_subprocess_routing_through_proxy.py` — 180 ms
...
`30_loop/test_subprocess_substrate_handler.py` — 139 ms
...
staged islet_sub==3.1.4 at /tmp/bubble-test-n0velbsh/vault/islet_sub/3.1.4/py3-none-any
...
`30_loop/test_substrate_routing_learns.py` — 87 ms
...
`70_project/test_project_freeze.py` — 95 ms
...
`70_project/test_project_ingest_basic.py` — 121 ms
...
`70_project/test_project_parent_chain.py` — 170 ms

```
spinoff delta: ('delta', '3.0.0', 'py3-none-any', '/tmp/bubble-test-iabq88ke/vault/delta/3.0.0/py3-none-any')
spinoff echo_pkg (inherited): ('echo-pkg', '4.0.0', 'py3-none-any', '/tmp/bubble-test-iabq88ke/vault/echo-pkg/4.0.0/py3-none-any')
```

## ✓ ingest persists package scope to shell metadata

`70_project/test_project_scope_persisted.py` — 140 ms
...
`70_project/test_project_soft_isolation.py` — 169 ms
