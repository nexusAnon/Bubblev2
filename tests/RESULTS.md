...
_Run: 2026-08-09T13:53:10 — 43 passed, 0 failed, 0 skipped._

---

## ✓ a fresh BUBBLE_HOME yields a usable vault DB on schema v2

`00_sanity/test_vault_initializes.py` — 47 ms

```
vault_db: /tmp/bubble-test-ukajzc2h/vault.db
...
AgentVault(home=av-second-home-o4o4_iu3) created separate vault root with its own SQLite index
```
...
`10_breakers/test_bundle_round_trip.py` — 199 ms
...
`10_breakers/test_canonical_name_validated.py` — 605 ms
...
`10_breakers/test_drift_refuses_and_records.py` — 81 ms
...
`10_breakers/test_empty_script_fetches_nothing.py` — 55 ms
...
`10_breakers/test_fetcher_refuses_off_host_url.py` — 50 ms
...
`10_breakers/test_flat_module_alias.py` — 90 ms

```
flatmod_old.__file__: /tmp/bubble-test-835z042s/vault/flatmod/1.0.0/py3-none-any/flatmod.py
flatmod_new.__file__: /tmp/bubble-test-835z042s/vault/flatmod/2.0.0/py3-none-any/flatmod.py
...
`10_breakers/test_https_index_required.py` — 55 ms
...
`10_breakers/test_importer_refuses_symlinks.py` — 73 ms
...
`10_breakers/test_late_alias_does_not_corrupt_earlier.py` — 88 ms
...
module id stable:  0x7f12a8712430 → 0x7f12a8712430
class id stable:   0x55d8dcd87b60 → 0x55d8dcd87b60
STATE dicts distinct: old@0x7f12a871a680 vs new@0x7f12a871af80
...
`10_breakers/test_metadata_per_alias.py` — 112 ms
...
`10_breakers/test_modules_and_deps_indexed.py` — 77 ms
...
`10_breakers/test_recursive_self_host.py` — 866 ms
...
`10_breakers/test_run_default_no_network.py` — 10 ms
...
`10_breakers/test_sdist_refused_by_default.py` — 615 ms
...
`10_breakers/test_shell_create_follows_requires_dist.py` — 112 ms

```
shell at /tmp/bubble-test-6mftqliy/shells/closuretest
...
`10_breakers/test_shell_create_from_manifest.py` — 197 ms
...
`10_breakers/test_shell_create_links_dist_info.py` — 328 ms

```
shell at /tmp/bubble-test-j4220vf3/shells/distinfotest
...
`10_breakers/test_shell_create_merges_namespace_packages.py` — 318 ms

```
shell at /tmp/bubble-test-bjb2z16v/shells/nstest
...
`10_breakers/test_shell_create_refuses_orphan_targets.py` — 161 ms
...
`10_breakers/test_top_level_carries_content_hash.py` — 133 ms
...
`10_breakers/test_top_level_contention_logged.py` — 127 ms
...
`10_breakers/test_top_level_index_bridges_import_to_dist.py` — 126 ms

```
distribution name: Carbohydrate-9000
top-level import:  sugar
top_level row:     ('Carbohydrate-9000', '3.0.0', 'sugar')
resolved module:   /tmp/bubble-test-prcir1gh/vault/Carbohydrate-9000/3.0.0/py3-none-any/sugar/__init__.py
...
`10_breakers/test_top_level_verify_mode.py` — 104 ms
...
`10_breakers/test_two_versions_one_process.py` — 142 ms

```
widget_old.Widget: id=0x55caa26d42b0
widget_new.Widget: id=0x55caa26d6100
...
`10_breakers/test_universal_v4.py` — 93 ms
...
`10_breakers/test_vault_dir_perms.py` — 5 ms

```
  bubble-test-wjgau0_i mode=0o700
  vault              mode=0o700
  .staging           mode=0o700
  shells             mode=0o700
  wheels             mode=0o700
...
`30_loop/test_agent_vault_isolation.py` — 185 ms
...
staged islet==3.1.4 at /tmp/bubble-test-5nj0sh7q/vault/islet/3.1.4/py3-none-any
...
`30_loop/test_probe_writes_host_toml.py` — 25 ms

```
probed_at: 2026-08-09T13:53:09.116861
...
`30_loop/test_subprocess_routing_through_proxy.py` — 177 ms
...
`30_loop/test_subprocess_substrate_handler.py` — 146 ms
...
staged islet_sub==3.1.4 at /tmp/bubble-test-y5k49b37/vault/islet_sub/3.1.4/py3-none-any
...
`30_loop/test_substrate_routing_learns.py` — 85 ms
...
`70_project/test_project_freeze.py` — 96 ms
...
`70_project/test_project_ingest_basic.py` — 102 ms
...
`70_project/test_project_parent_chain.py` — 128 ms

```
spinoff delta: ('delta', '3.0.0', 'py3-none-any', '/tmp/bubble-test-gad5ohc6/vault/delta/3.0.0/py3-none-any')
spinoff echo_pkg (inherited): ('echo-pkg', '4.0.0', 'py3-none-any', '/tmp/bubble-test-gad5ohc6/vault/echo-pkg/4.0.0/py3-none-any')
```

## ✓ ingest persists package scope to shell metadata

`70_project/test_project_scope_persisted.py` — 92 ms
...
`70_project/test_project_soft_isolation.py` — 123 ms
