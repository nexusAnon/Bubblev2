...
def body(r: Result):
    ...
    mock_macho.extend(struct.pack("<I", MH_MAGIC_64)) # magic
    mock_macho.extend(struct.pack("<IIIIIII", 0x01000007, 3, 2, 1, 32, 0x00200000, 0))

    # Append LC_RPATH load command
    # cmd (4), cmdsize (4), path offset (4)
    # RPATH string: "/usr/lib" + padding to 8 bytes alignment
    cmd_data = bytearray()
    cmd_data.extend(struct.pack("<III", LC_RPATH, 24, 12))
